...
def create_fastwam(
    ...
    if isinstance(langforce_mvp, DictConfig):
        langforce_mvp = OmegaConf.to_container(langforce_mvp, resolve=True)
    if langforce_mvp is None:
        langforce_mvp = {}
    if not isinstance(langforce_mvp, dict):
        raise ValueError(
