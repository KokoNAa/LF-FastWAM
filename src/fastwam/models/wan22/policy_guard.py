...
class ERAFActionContextInjector(nn.Module):
    """Project ERAF queries into Action-Expert conditioning tokens.

    This module changes conditioning rather than decoded actions. The Base
    candidate keeps its original language/state context. A counterfactual
    candidate appends these bounded ERAF tokens and is generated by the same
    Action Expert from the same initial diffusion noise. No post-sampler action
    residual is produced by this module.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if min(goal_dim, text_dim, hidden_dim) <= 0:
            raise ValueError("ERAF action-context dimensions must be positive.")
        if not 0.0 < float(initial_scale) < 1.0:
            raise ValueError(
                "ERAF action-context initial scale must lie in (0, 1)."
            )
        self.goal_dim = int(goal_dim)
        self.text_dim = int(text_dim)
        self.hidden_dim = int(hidden_dim)
        ...
        initial_logit = math.log(
            float(initial_scale) / (1.0 - float(initial_scale))
        )
        self.scale_logit = nn.Parameter(torch.tensor(initial_logit))
    ...
    def forward(
        ...
        *,
        ...
        if context.ndim != 3 or context.shape[-1] != self.text_dim:
            raise ValueError(
                "ERAF action context must have shape "
                f"[B,L,{self.text_dim}], got {tuple(context.shape)}."
            )
        if (
            context_mask.ndim != 2
            ...
            raise ValueError(
                "ERAF action context mask must be boolean [B,L]."
            )
        if (
            goal_queries.ndim != 3
            or goal_queries.shape[0] != context.shape[0]
            or goal_queries.shape[-1] != self.goal_dim
        ):
            raise ValueError(
