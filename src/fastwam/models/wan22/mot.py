...
class MoT(nn.Module):
    ...
    def _prepare_context_payload(
        ...
        """Make all-masked cross-attention rows backend-safe once per forward."""
        if context_payload is None or context_payload.get("context") is None:
        ...
        if context_mask.dtype != torch.bool:
            raise ValueError(
                f"Cross-attention mask must be boolean, got {context_mask.dtype}."
            )
        if context_mask.ndim not in (2, 3, 4):
            raise ValueError(
                "Cross-attention mask must be [B,L], [B,S,L], or [B,1,S,L], "
                f"got shape {tuple(context_mask.shape)}."
            )
        if context_mask.shape[-1] == 0:
            raise ValueError("Cross-attention context must contain at least one token.")

        valid_rows = context_mask.any(dim=-1, keepdim=True)
    ...
    def _apply_expert_post_block(
        ...
        if context_payload is not None:
            ...
            if context is not None:
                ...
                valid_rows = context_payload.get("valid_rows")
                if valid_rows is not None:
                    if valid_rows.ndim == 4:
                        valid_rows = valid_rows.squeeze(1)
                    if valid_rows.ndim == 2:
                        valid_rows = valid_rows.unsqueeze(1)
                    cross_out = cross_out * valid_rows.to(dtype=cross_out.dtype)
                x = x + cross_out
    ...
    def prefill_video_cache(
        ...
        *,
        ...
        x = video_tokens
        video_context_payload = self._prepare_context_payload(
