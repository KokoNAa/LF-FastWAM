...
if version == 9:
    ...
    if objective not in set(range(1, 37)):
        raise SystemExit(
    ...
    if objective >= 29 and (
        ...
        != ("injector_only_with_frozen_v928_teacher_and_gate" if objective in {30, 31, 32, 33, 34, 35, 36, 37}
    ...
    if objective in {30, 31, 32, 33, 34, 35, 36, 37}:
        ...
        if objective in {31, 32, 33, 34, 35, 36, 37} and (
            ...
            raise SystemExit(
                f"V9.{objective} checkpoint lacks selective full-goal preservation"
            )
        if objective == 33:
            from fastwam.utils.cf_ablation import POSITIVE_ONLY_RANKING_CONTRACT
            if (
                metadata.get("eraf_corrective_ranking_gradient_contract")
        ...
        if objective in {34, 35, 36, 37}:
            ...
            if (
                metadata.get("eraf_paired_ranking_gradient_contract")
                != UNIVERSAL_POSITIVE_ONLY_RANKING_CONTRACT
            ):
                raise SystemExit(
        ...
        if objective in {35, 36, 37}:
            ...
            expected_semantic_contract = (
                SOFT_ACTION_VIOLATION_SEMANTIC_CONTRAST_CONTRACT
                if objective == 37
                else ACTION_VIOLATION_GATED_SEMANTIC_CONTRAST_CONTRACT
                if objective == 36
                else PAIRED_SEMANTIC_CONTRAST_CONTRACT
            )
            if (
                ...
                raise SystemExit(
                    f"V9.{objective} checkpoint lacks paired semantic contrast contract"
                )
            if objective == 37 and (
                float(
                    metadata.get("eraf_paired_semantic_non_violation_weight")
                    or 0
                )
                != 0.5
            ):
                raise SystemExit(
                    "V9.37 checkpoint lacks soft semantic non-violation weight"
                )
    if pretrained_eraf_joint:
...
mapping = {
    ...
    "eraf_paired_semantic_non_violation_weight": (
