#!/usr/bin/env python3
"""Fresh matched four-arm trial with initial expert anchors and restored old-task weights."""
...
def main():
    ...
    diagnostic=source.with_name('20260908-expanded60-action-fit-v1')
    completed_source(diagnostic)
    if not read(diagnostic/'comparison.json')['matched_actual_inputs']:raise ValueError('Unpaired action diagnostic.')
    plan=deepcopy(old)
    plan.update(format='robotwin_initial_anchor_fourarm_v1',code_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=REPO,text=True).strip(),
        ...
        action_recipe='Fresh200 from the same strongest action policy and audited zero-output semantic1500 parents. One initial expert, two ordinary expert, two Correct retention, four CF retention, three FG or matched ordinary slots. No continuation from regressed action weights.',
    ...
    if shutil.disk_usage(root.parent).free<3*1024**3+plan['reserved_output_bytes']:
        raise ValueError('Measured final outputs and three-GiB reserve do not fit.')
