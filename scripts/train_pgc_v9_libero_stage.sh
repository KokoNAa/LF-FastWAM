...
case "${STAGE}" in
  ...
  action-waypoint)
    START_STEP=16250
    STAGE_START_STEP=16250
    DEFAULT_STAGE_STEPS=1000
    LEARNING_RATE="2.0e-5"
    CONFIG_STAGE="action"
    DEFAULT_GROUNDING_OBJECTIVE_VERSION=20
    SAVE_EVERY=250
    ;;
  action-expert-alignment)
    START_STEP=17250
    STAGE_START_STEP=17250
    DEFAULT_STAGE_STEPS=1000
    LEARNING_RATE="2.0e-5"
    CONFIG_STAGE="action"
  ...
  action-clause-ranking)
    ...
    SAVE_EVERY=250
    ;;
  action-alignment-preserving-clause)
    # Deliberately branch from the admitted V9.21 checkpoint rather than
    # continuing the alignment-regressing clause-ranking checkpoint.
    START_STEP=18250
    STAGE_START_STEP=18250
    DEFAULT_STAGE_STEPS=500
    LEARNING_RATE="1.0e-5"
    CONFIG_STAGE="action"
    DEFAULT_GROUNDING_OBJECTIVE_VERSION=23
    SAVE_EVERY=250
    ;;
  action-isolated-clause-residual)
    # Branch directly from admitted V9.21. The expert-aligned positive route is
    # immutable; a zero-init residual learns only coherent wrong-clause fallback.
    START_STEP=18250
    STAGE_START_STEP=18250
    DEFAULT_STAGE_STEPS=500
    LEARNING_RATE="1.0e-5"
    CONFIG_STAGE="action"
    DEFAULT_GROUNDING_OBJECTIVE_VERSION=24
    SAVE_EVERY=250
    ;;
  action-context-injection)
    # Single-variable replacement of the action interface. Warm-start from
    # V9.24, freeze every existing ERAF/Proposal/residual component, and train
    # only ERAF tokens injected inside the shared Action Expert denoiser.
    START_STEP=18750
    STAGE_START_STEP=18750
    DEFAULT_STAGE_STEPS=1000
    LEARNING_RATE="2.0e-5"
    CONFIG_STAGE="action"
    DEFAULT_GROUNDING_OBJECTIVE_VERSION=25
    SAVE_EVERY=250
    ;;
  action-eraf-expert-lora)
    # Keep the admitted ERAF/completion memory frozen. Adapt the one deployed
    # Video/Action Expert path with LoRA plus the internal ERAF context injector.
    START_STEP=19750
    STAGE_START_STEP=19750
    DEFAULT_STAGE_STEPS=1000
    LEARNING_RATE="5.0e-6"
    CONFIG_STAGE="action"
    DEFAULT_GROUNDING_OBJECTIVE_VERSION=26
    SAVE_EVERY=250
    ;;
  action)
  ...
  verifier)
    ...
    ;;
  *)
    echo "Stage must include a supported grounding/action stage; got ${STAGE}." >&2
    exit 1
    ;;
esac
...
case "${STAGE}" in
  ...
  action-phase-servo)
    ...
    ;;
  action-waypoint)
    if [[ "${GROUNDING_OBJECTIVE_VERSION}" != "20" ]]; then
      echo "Formal V9.20 phase-compatible waypoint training requires objective version 20." >&2
      exit 1
    fi
    ;;
  action-expert-alignment)
    if [[ "${GROUNDING_OBJECTIVE_VERSION}" != "21" ]]; then
      echo "Formal expert-prefix residual alignment requires objective version 21." >&2
      exit 1
    fi
    ;;
  action-clause-ranking)
    ...
    ;;
  action-alignment-preserving-clause)
    if [[ "${GROUNDING_OBJECTIVE_VERSION}" != "23" ]]; then
      echo "Formal alignment-preserving clause ranking requires objective version 23." >&2
      exit 1
    fi
    ;;
  action-isolated-clause-residual)
    if [[ "${GROUNDING_OBJECTIVE_VERSION}" != "24" ]]; then
      echo "Formal isolated clause semantic-residual training requires objective version 24." >&2
      exit 1
    fi
    ;;
  action-context-injection)
    if [[ "${GROUNDING_OBJECTIVE_VERSION}" != "25" ]]; then
      echo "Formal internal ERAF Action-Expert context injection requires objective version 25." >&2
      exit 1
    fi
    ;;
  action-eraf-expert-lora)
    if [[ "${GROUNDING_OBJECTIVE_VERSION}" != "26" ]]; then
      echo "Formal single-path ERAF shared Expert-LoRA training requires objective version 26." >&2
      exit 1
    fi
    ;;
  grounding-role)
...
if [[ "${STAGE}" == "grounding-phase-rebinding" || "${STAGE}" == "grounding-phase-memory" || "${STAGE}" == "action-completion-only" || "${STAGE}" == "action-geometry-causal" || "${STAGE}" == "action-semantic-causal" || "${STAGE}" == "action-direct-geometry" || "${STAGE}" == "action-phase-residual" || "${STAGE}" == "action-phase-servo" || "${STAGE}" == "action-waypoint" || "${STAGE}" == "action-expert-alignment" || "${STAGE}" == "action-clause-ranking" || "${STAGE}" == "action-alignment-preserving-clause" || "${STAGE}" == "action-isolated-clause-residual" || "${STAGE}" == "action-context-injection" || "${STAGE}" == "action-eraf-expert-lora" ]]; then
  ...
  if [[ -z "${CLOSED_LOOP_GROUNDING_SIDECAR}" || ! -d "${CLOSED_LOOP_GROUNDING_SIDECAR}" ]]; then
    echo "Phase-aware V9 action stages require PGC_V9_CLOSED_LOOP_GROUNDING_SIDECAR." >&2
    exit 1
  fi
fi
...
if [[ "${GROUNDING_OBJECTIVE_VERSION}" == "14" || "${GROUNDING_OBJECTIVE_VERSION}" == "15" || "${GROUNDING_OBJECTIVE_VERSION}" == "16" || "${GROUNDING_OBJECTIVE_VERSION}" == "17" || "${GROUNDING_OBJECTIVE_VERSION}" == "18" || "${GROUNDING_OBJECTIVE_VERSION}" == "19" || "${GROUNDING_OBJECTIVE_VERSION}" == "20" || "${GROUNDING_OBJECTIVE_VERSION}" == "21" || "${GROUNDING_OBJECTIVE_VERSION}" == "22" || "${GROUNDING_OBJECTIVE_VERSION}" == "25" ]]; then
  # V9.13+ freeze the validated V9.11 perception/grounding losses. Later action
  # stages inherit V9.14's exact zero-loss contract while calibrating action paths.
  MASK_WEIGHT=0.0
...
if [[ "${GROUNDING_OBJECTIVE_VERSION}" == "19" || "${GROUNDING_OBJECTIVE_VERSION}" == "20" || "${GROUNDING_OBJECTIVE_VERSION}" == "21" || "${GROUNDING_OBJECTIVE_VERSION}" == "22" ]]; then
...
if stage == "grounding":
    ...
    objective_upgrade = (
        ...
        or (
            stage == "action-waypoint"
            and grounding_objective_version == 19
            and int(requested_objective) == 20
        )
        or (
            stage == "action-expert-alignment"
        ...
        or (
            stage == "action-alignment-preserving-clause"
            and grounding_objective_version == 21
            and int(requested_objective) == 23
        )
        or (
            stage == "action-isolated-clause-residual"
            and grounding_objective_version == 21
            and int(requested_objective) == 24
        )
        or (
            stage == "action-context-injection"
            and grounding_objective_version == 24
            and int(requested_objective) == 25
        )
        or (
            stage == "action-eraf-expert-lora"
            and grounding_objective_version == 25
            and int(requested_objective) == 26
        )
    )
    ...
    expected_input_stage = {
        ...
        "action-alignment-preserving-clause": "action",
        "action-isolated-clause-residual": "action",
        "action-context-injection": "action",
        "action-eraf-expert-lora": "action",
        "verifier": "action",
    }[stage]
    saved_stage = str(
        (payload.get("architecture_metadata") or {}).get(
    ...
    if stage == "action-waypoint":
        ...
        if (
            ...
            raise SystemExit(
                "V9.20 must warm-start from the completed V9.19 phase-servo "
                "checkpoint at step 16250."
            )
    if stage == "action-expert-alignment":
        metadata = payload.get("architecture_metadata") or {}
        if (
    ...
    if stage == "action-alignment-preserving-clause":
        metadata = payload.get("architecture_metadata") or {}
        if (
            grounding_objective_version != 21
            or metadata.get("eraf_training_stage") != "action"
            ...
            raise SystemExit(
                "Alignment-preserving clause ranking must branch directly "
                "from the completed expert-alignment checkpoint at step 18250."
            )
    if stage == "action-isolated-clause-residual":
        metadata = payload.get("architecture_metadata") or {}
        if (
            grounding_objective_version != 21
            or metadata.get("eraf_training_stage") != "action"
            ...
            raise SystemExit(
                "Isolated clause semantic-residual training must branch "
                "directly from the completed expert-alignment checkpoint at "
                "step 18250."
            )
    if stage == "action-context-injection":
        metadata = payload.get("architecture_metadata") or {}
        if (
            grounding_objective_version != 24
            or metadata.get("eraf_training_stage") != "action"
            or not bool(metadata.get("eraf_action_joint_training", False))
            or metadata.get("eraf_action_joint_contract")
            ...
            raise SystemExit(
                "Internal ERAF Action-Expert context injection must warm-start "
                "from the completed V9.24 checkpoint at step 18750."
            )
    if stage == "action-eraf-expert-lora":
        metadata = payload.get("architecture_metadata") or {}
        if (
            grounding_objective_version != 25
            or metadata.get("eraf_training_stage") != "action"
            or not bool(metadata.get("eraf_action_joint_training", False))
            or metadata.get("eraf_action_joint_contract")
            ...
            or metadata.get("eraf_role_adapter_trainable_scope")
            != "eraf_action_context_injector_only"
            or metadata.get("eraf_action_context_injection_contract")
...
if [[ "${STAGE}" == "grounding-phase-memory" || "${STAGE}" == "action-completion-only" || "${STAGE}" == "action-geometry-causal" || "${STAGE}" == "action-semantic-causal" || "${STAGE}" == "action-direct-geometry" || "${STAGE}" == "action-phase-residual" || "${STAGE}" == "action-phase-servo" || "${STAGE}" == "action-waypoint" || "${STAGE}" == "action-expert-alignment" || "${STAGE}" == "action-clause-ranking" || "${STAGE}" == "action-alignment-preserving-clause" || "${STAGE}" == "action-isolated-clause-residual" || "${STAGE}" == "action-context-injection" || "${STAGE}" == "action-eraf-expert-lora" ]]; then
  PHASE_SAFE_MEMORY=true
else
  PHASE_SAFE_MEMORY=false
fi
if [[ "${STAGE}" == "action-completion-only" || "${STAGE}" == "action-geometry-causal" || "${STAGE}" == "action-semantic-causal" || "${STAGE}" == "action-direct-geometry" || "${STAGE}" == "action-phase-residual" || "${STAGE}" == "action-phase-servo" || "${STAGE}" == "action-waypoint" || "${STAGE}" == "action-expert-alignment" || "${STAGE}" == "action-clause-ranking" || "${STAGE}" == "action-alignment-preserving-clause" || "${STAGE}" == "action-isolated-clause-residual" || "${STAGE}" == "action-context-injection" || "${STAGE}" == "action-eraf-expert-lora" ]]; then
  COMPLETION_ONLY_MEMORY=true
  ACTION_JOINT_TRAINING=true
  if [[ "${STAGE}" == "action-eraf-expert-lora" ]]; then
    GROUNDING_AUX_WEIGHT="${PGC_V9_GROUNDING_AUX_WEIGHT:-0.25}"
  else
...
if [[ "${STAGE}" == "action-eraf-expert-lora" ]]; then
  EXPERT_LORA_ENABLED=true
else
  EXPERT_LORA_ENABLED=false
fi
EXPERT_LORA_RANK="${PGC_V9_EXPERT_LORA_RANK:-16}"
EXPERT_LORA_ALPHA="${PGC_V9_EXPERT_LORA_ALPHA:-16}"
EXPERT_LORA_DROPOUT="${PGC_V9_EXPERT_LORA_DROPOUT:-0.05}"
EXPERT_LORA_WORLD_LANGUAGE_WEIGHT="${PGC_V9_EXPERT_LORA_WORLD_LANGUAGE_WEIGHT:-0.10}"
EXPERT_LORA_WORLD_LANGUAGE_MARGIN="${PGC_V9_EXPERT_LORA_WORLD_LANGUAGE_MARGIN:-0.01}"
EXPERT_LORA_NATIVE_ACTION_WEIGHT="${PGC_V9_EXPERT_LORA_NATIVE_ACTION_WEIGHT:-1.0}"
EXPERT_LORA_COUNTERFACTUAL_ACTION_WEIGHT="${PGC_V9_EXPERT_LORA_COUNTERFACTUAL_ACTION_WEIGHT:-1.0}"
EXPERT_LORA_REGULARIZATION_WEIGHT="${PGC_V9_EXPERT_LORA_REGULARIZATION_WEIGHT:-1.0e-6}"
...
ACTION_WAYPOINT_COMPATIBILITY_WEIGHT="${PGC_V9_ACTION_WAYPOINT_COMPATIBILITY_WEIGHT:-1.0}"
ACTION_WAYPOINT_IMITATION_WEIGHT="${PGC_V9_ACTION_WAYPOINT_IMITATION_WEIGHT:-2.0}"
ACTION_WAYPOINT_DIRECTION_WEIGHT="${PGC_V9_ACTION_WAYPOINT_DIRECTION_WEIGHT:-0.5}"
ACTION_WAYPOINT_ZERO_WEIGHT="${PGC_V9_ACTION_WAYPOINT_ZERO_WEIGHT:-1.0}"
ACTION_WAYPOINT_MIN_COSINE="${PGC_V9_ACTION_WAYPOINT_MIN_COSINE:-0.20}"
ACTION_WAYPOINT_TANGENT_MAX_RATIO="${PGC_V9_ACTION_WAYPOINT_TANGENT_MAX_RATIO:-0.75}"
ACTION_EXPERT_IMITATION_WEIGHT="${PGC_V9_ACTION_EXPERT_IMITATION_WEIGHT:-2.0}"
ACTION_EXPERT_DIRECTION_WEIGHT="${PGC_V9_ACTION_EXPERT_DIRECTION_WEIGHT:-0.5}"
...
ACTION_CLAUSE_TEACHER_WEIGHT="${PGC_V9_ACTION_CLAUSE_TEACHER_WEIGHT:-4.0}"
ACTION_CLAUSE_ALIGNMENT_GUARD_WEIGHT="${PGC_V9_ACTION_CLAUSE_ALIGNMENT_GUARD_WEIGHT:-8.0}"
ACTION_CLAUSE_WRONG_SUPPRESSION_WEIGHT="${PGC_V9_ACTION_CLAUSE_WRONG_SUPPRESSION_WEIGHT:-4.0}"
...
echo "  alignment_preservation=teacher:${ACTION_CLAUSE_TEACHER_WEIGHT} nonregression:${ACTION_CLAUSE_ALIGNMENT_GUARD_WEIGHT} correct_rank_detached:true"
echo "  isolated_clause_residual=wrong_suppression:${ACTION_CLAUSE_WRONG_SUPPRESSION_WEIGHT} positive_route:frozen_v921_exact"
if [[ "${STAGE}" == "action-context-injection" ]]; then
  echo "  action_context_injection=shared_action_expert:true trainable_scope:eraf_action_context_injector_only post_action_residual:false native_action_supervision:false"
fi
if [[ "${STAGE}" == "action-eraf-expert-lora" ]]; then
  echo "  eraf_expert_lora=single_path:true video_action_lora:true rank:${EXPERT_LORA_RANK} alpha:${EXPERT_LORA_ALPHA} dropout:${EXPERT_LORA_DROPOUT} post_action_residual:false"
  echo "  eraf_expert_supervision=world_flow:1 wrong_language:${EXPERT_LORA_WORLD_LANGUAGE_WEIGHT} native_action:${EXPERT_LORA_NATIVE_ACTION_WEIGHT} counterfactual_action:${EXPERT_LORA_COUNTERFACTUAL_ACTION_WEIGHT} eraf_preservation:${GROUNDING_AUX_WEIGHT}"
fi
...
RUN_ID="pgc-${RUN_TAG}" exec bash scripts/train_zero1.sh "${NPROC_PER_NODE}" \
  ...
  "model.policy_guard.entity_relation_grounding.expert_lora_world_language_weight=${EXPERT_LORA_WORLD_LANGUAGE_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.expert_lora_world_language_margin=${EXPERT_LORA_WORLD_LANGUAGE_MARGIN}" \
  "model.policy_guard.entity_relation_grounding.expert_lora_native_action_weight=${EXPERT_LORA_NATIVE_ACTION_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.expert_lora_counterfactual_action_weight=${EXPERT_LORA_COUNTERFACTUAL_ACTION_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.expert_lora_regularization_weight=${EXPERT_LORA_REGULARIZATION_WEIGHT}" \
  ...
  "model.policy_guard.entity_relation_grounding.action_waypoint_compatibility_weight=${ACTION_WAYPOINT_COMPATIBILITY_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.action_waypoint_imitation_weight=${ACTION_WAYPOINT_IMITATION_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.action_waypoint_direction_weight=${ACTION_WAYPOINT_DIRECTION_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.action_waypoint_zero_weight=${ACTION_WAYPOINT_ZERO_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.action_waypoint_min_cosine=${ACTION_WAYPOINT_MIN_COSINE}" \
  "model.policy_guard.entity_relation_grounding.action_waypoint_tangent_max_ratio=${ACTION_WAYPOINT_TANGENT_MAX_RATIO}" \
  "model.policy_guard.entity_relation_grounding.action_expert_imitation_weight=${ACTION_EXPERT_IMITATION_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.action_expert_direction_weight=${ACTION_EXPERT_DIRECTION_WEIGHT}" \
  ...
  "model.policy_guard.entity_relation_grounding.action_clause_teacher_weight=${ACTION_CLAUSE_TEACHER_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.action_clause_alignment_guard_weight=${ACTION_CLAUSE_ALIGNMENT_GUARD_WEIGHT}" \
  "model.policy_guard.entity_relation_grounding.action_clause_wrong_suppression_weight=${ACTION_CLAUSE_WRONG_SUPPRESSION_WEIGHT}" \
  ...
  "model.lora.enabled=${EXPERT_LORA_ENABLED}" \
  "model.lora.rank=${EXPERT_LORA_RANK}" \
  "model.lora.alpha=${EXPERT_LORA_ALPHA}" \
  "model.lora.dropout=${EXPERT_LORA_DROPOUT}" \
