...
def report(root, *, draws=10000):
    ...
    domain = plan.get('task_config', 'demo_clean')
    if domain not in ('demo_clean', 'demo_randomized'): raise ValueError('Unknown evaluation domain.')
    ...
    for task in TASKS:
        catalog = root/'catalog'/task/domain/'correct/episodes.jsonl'
        ...
        for arm in ARMS:
            ...
            if complete.get('task_config', 'demo_clean') != domain:
    ...
    return dict(format='robotwin_five_task_paired_comparison_v1', complete=True, episodes=15*n,
        task_config=domain, episodes_per_cell=n, cells=cells, source_sha256=sources,
...
def markdown(result):
    ...
    text = [f'# 五任务三组配对比较：{scope}', '',
        f'环境 {result.get("task_config", "demo_clean")}；每任务每模型 {n} 个配对场景，总计 {15*n} 回合；五任务等权。', '',
