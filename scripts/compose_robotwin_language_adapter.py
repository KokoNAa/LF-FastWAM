...
def compose(anchor, repair):
    ...
    result = dict(anchor)
    result['mot_trainable'] = {name: target[name] if name in language else value for name, value in source.items()}
...
def main():
    ...
    result, language = compose(torch.load(args.anchor, map_location='cpu', weights_only=False),
                               torch.load(args.repair, map_location='cpu', weights_only=False))
    result['decision_repair'] = {'composition': 'language_adapters_from_repair_other_adapters_from_anchor',
    ...
    print(f'[composed] language={len(language)}/{len(result["mot_trainable"])} output={output}')
