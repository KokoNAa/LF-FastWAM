...
NPROC_PER_NODE="${1:?Usage: bash scripts/train_tc_counterfactual_action_object.sh <gpus> <tc_v4_or_v5_checkpoint> [seed] [max_steps]}"
...
RUN_TAG="${RUN_TAG:-v5-cf-action-positive-object-${MAX_STEPS}-seed${TRAIN_SEED}-v1}"
...
if payload.get("format") != "fastwam_lora_adapter_v1":
    raise SystemExit("TC v5 requires a FastWAM LoRA adapter checkpoint")
if metadata.get("architecture") != "tc_fastwam":
    raise SystemExit("TC v5 must initialize from a protected TC checkpoint")
version = int(metadata.get("transition_contract_version", -1))
if version not in {4, 5}:
    raise SystemExit(f"TC v5 requires v4/v5 initialization, got v{version}")
...
if version == 5 and not metadata.get("use_cf_action_positive"):
    raise SystemExit("TC v5 checkpoint is missing action-positive metadata")
...
echo "[TC-FastWAM] TC-Full v5 counterfactual-action-positive training"
...
echo "  method=V4 + same-state alternate policy branch + demonstrated-action EMA positives"

RUN_ID="tc-cap-${RUN_TAG}" bash scripts/train_zero1.sh "${NPROC_PER_NODE}" \
  "task=${TASK_NAME}" \
  ...
  model.transition_contract.version=5 \
...
echo "[TC-FastWAM] TC-Full v5 training complete."
