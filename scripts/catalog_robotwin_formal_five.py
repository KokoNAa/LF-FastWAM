...
def main():
    ...
    from experiments.robotwin.language_interventions import (
        EPISODE_FORMAT, load_intervention_manifest, select_intervention_pair, evaluate_goal)
        ...
        for seed in range(args.start_seed, args.start_seed + args.max_attempts):
            ...
            try:
                for variant in (spec.source_variant, spec.counterfactual_variant):
                    ...
                    if any(evaluate_goal(task, g).success for g in (pair.source_goal, pair.counterfactual_goal)):
                        raise ValueError('An initial benchmark goal is already satisfied')
                    ...
                    selected = pair.source_goal if variant == spec.source_variant else pair.counterfactual_goal
                    if not evaluate_goal(task, selected).success:
                        raise ValueError('Expert endpoint fails actual benchmark predicate')
                ...
                counterfactual = (str(pair.counterfactual_instruction) if pair.counterfactual_instruction is not None
