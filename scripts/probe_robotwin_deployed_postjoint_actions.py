#!/usr/bin/env python3
"""Audit deployed-rollout action models on the same fixed states as the prior objective and strongest control."""
...
def live(pid,source):
    ...
    return stat[0]!='Z' and str(source).encode() in cmd and b'run_robotwin_deployed_action_trial.py' in cmd
...
def main():
    ...
    for model in models.values():
        if read(model.parent/'plan.json').get('action_objective')!='deployed_rollout_v1':
            raise ValueError('Expected a completed deployed-rollout objective model.')
    bindings={str(p):file_sha256(p) for p in [source/'protocol.json',Path(plan['manifest']),*models.values(),*references]}
