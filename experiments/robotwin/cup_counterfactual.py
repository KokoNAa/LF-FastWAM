...
def behind_goal(cup_point, coaster_point, floor_z, upright_dot, left_open, right_open):
    delta = np.asarray(cup_point) - np.asarray(coaster_point)
    return bool(abs(delta[0]) < .035 and .095 < delta[1] < .165
                and abs(float(cup_point[2]) - floor_z) < .015
                and upright_dot > .95 and left_open and right_open)


def initialize_geometry(task):
