...
def model_for(objective=30, source=None, ablation="none"):
    return tiny_pgc_fastwam(
        ...
        v9_safe_gain_injector_training_steps=250 if objective in {30, 31, 32, 33, 34, 35, 36, 37} else 0,
        v9_safe_gain_gate_calibration_steps=0,
        v9_safe_gain_noise_levels=2 if objective in {30, 31, 32, 33, 34, 35, 36, 37} else 1,
        v9_cf_ablation=ablation,
        v9_full_goal_action_preservation_weight=1.0 if objective in {31, 32, 33, 34, 35, 36, 37} else 0.0,
        v9_full_goal_token_preservation_weight=0.1 if objective in {31, 32, 33, 34, 35, 36, 37} else 0.0,
        v9_full_goal_context_preservation_weight=1.0 if objective in {31, 32, 33, 34, 35, 36, 37} else 0.0,
        v9_paired_semantic_contrast_weight=0.1 if objective in {35, 36, 37} else 0.0,
        v9_paired_semantic_contrast_margin=0.1,
        v9_paired_semantic_non_violation_weight=0.5,
    )


@pytest.mark.parametrize("objective", [29, 30, 31, 32, 33, 34, 35, 36, 37])
...
def test_v933_hydra_keeps_v932_route_and_selects_positive_gradient():
    with initialize_config_dir(
        config_dir=str(Path(__file__).resolve().parents[1] / "configs"),
        version_base=None,
    ):
        cfg = compose(
            config_name="train", overrides=["task=libero_eraf_safe_gain_v933_2cam224"]
        )
    eraf = cfg.model.policy_guard.entity_relation_grounding
    assert eraf.grounding_objective_version == 33
    assert eraf.cf_ablation == "mask_lift_ranking"
    assert eraf.safe_gain_injector_training_steps == cfg.max_steps == 50
    assert cfg.save_every == 25


def test_v934_hydra_keeps_masks_and_selects_universal_positive_gradient():
    with initialize_config_dir(
        config_dir=str(Path(__file__).resolve().parents[1] / "configs"),
        version_base=None,
    ):
        cfg = compose(
            config_name="train", overrides=["task=libero_eraf_safe_gain_v934_2cam224"]
        )
    eraf = cfg.model.policy_guard.entity_relation_grounding
    assert eraf.grounding_objective_version == 34
    assert eraf.cf_ablation == "mask_lift_ranking"
    assert eraf.safe_gain_injector_training_steps == cfg.max_steps == 50
    assert cfg.save_every == 25


def test_v935_hydra_adds_bidirectional_semantic_contrast():
    with initialize_config_dir(
        config_dir=str(Path(__file__).resolve().parents[1] / "configs"),
        version_base=None,
    ):
        cfg = compose(
            config_name="train", overrides=["task=libero_eraf_safe_gain_v935_2cam224"]
        )
    eraf = cfg.model.policy_guard.entity_relation_grounding
    assert eraf.grounding_objective_version == 35
    assert eraf.cf_ablation == "mask_lift_ranking"
...
def test_v937_hydra_soft_weights_non_violation_semantics():
    with initialize_config_dir(
        config_dir=str(Path(__file__).resolve().parents[1] / "configs"),
        version_base=None,
    ):
        cfg = compose(
            config_name="train", overrides=["task=libero_eraf_safe_gain_v937_2cam224"]
        )
    eraf = cfg.model.policy_guard.entity_relation_grounding
    ...
    assert eraf.paired_semantic_contrast_weight == pytest.approx(0.1)
    assert eraf.paired_semantic_contrast_margin == pytest.approx(0.1)
    assert eraf.paired_semantic_non_violation_weight == pytest.approx(0.5)
    assert eraf.safe_gain_injector_training_steps == cfg.max_steps == 25
    assert cfg.save_every == 5
...
def test_v933_runner_reuses_v932_workflow_with_new_objective(tmp_path):
    root = Path(__file__).resolve().parents[1]
    path = root / "scripts/train_libero_eraf_safe_gain_v932.py"
    spec = importlib.util.spec_from_file_location("verified_ranking_launch", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    with initialize_config_dir(config_dir=str(root / "configs"), version_base=None):
        source = compose(
    ...
    source.batch_size = 1
    source.gradient_accumulation_steps = 4
    derived = module.training_config(source, tmp_path / "v933", 4, objective=33)
    command = module.training_command(
        tmp_path / "v933", 4, derived, objective=33
    )
    derived_eraf = derived.model.policy_guard.entity_relation_grounding
    assert derived_eraf.grounding_objective_version == 33
    assert derived_eraf.cf_ablation == "mask_lift_ranking"
    assert derived.gradient_accumulation_steps == 3
    assert command[-1] == "v933_train"


def test_v934_runner_reuses_v932_workflow_with_universal_contract(tmp_path):
    root = Path(__file__).resolve().parents[1]
    path = root / "scripts/train_libero_eraf_safe_gain_v932.py"
    spec = importlib.util.spec_from_file_location("universal_ranking_launch", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    with initialize_config_dir(config_dir=str(root / "configs"), version_base=None):
        source = compose(
    ...
    source.batch_size = 1
    source.gradient_accumulation_steps = 4
    derived = module.training_config(source, tmp_path / "v934", 4, objective=34)
    command = module.training_command(
        tmp_path / "v934", 4, derived, objective=34
    )
    derived_eraf = derived.model.policy_guard.entity_relation_grounding
    assert derived_eraf.grounding_objective_version == 34
    assert derived_eraf.cf_ablation == "mask_lift_ranking"
    assert derived.gradient_accumulation_steps == 3
    assert command[-1] == "v934_train"
    assert (
        module.ranking_gradient_contract(34)
        == ablation.UNIVERSAL_POSITIVE_ONLY_RANKING_CONTRACT
    )


def test_v935_runner_adds_portable_semantic_contrast_contract(tmp_path):
    root = Path(__file__).resolve().parents[1]
    path = root / "scripts/train_libero_eraf_safe_gain_v932.py"
    spec = importlib.util.spec_from_file_location("semantic_contrast_launch", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    with initialize_config_dir(config_dir=str(root / "configs"), version_base=None):
        source = compose(
    ...
    source.batch_size = 1
    source.gradient_accumulation_steps = 4
    derived = module.training_config(source, tmp_path / "v935", 4, objective=35)
    command = module.training_command(
        tmp_path / "v935", 4, derived, objective=35
    )
    derived_eraf = derived.model.policy_guard.entity_relation_grounding
    assert derived_eraf.grounding_objective_version == 35
    assert derived_eraf.cf_ablation == "mask_lift_ranking"
    assert derived_eraf.paired_semantic_contrast_weight == pytest.approx(0.1)
    assert derived_eraf.paired_semantic_contrast_margin == pytest.approx(0.1)
    assert derived.gradient_accumulation_steps == 3
    assert command[-1] == "v935_train"
    assert (
...
def test_v937_runner_uses_soft_semantics_and_early_checkpoints(tmp_path):
    root = Path(__file__).resolve().parents[1]
    path = root / "scripts/train_libero_eraf_safe_gain_v932.py"
    spec = importlib.util.spec_from_file_location("soft_semantic_launch", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    with initialize_config_dir(config_dir=str(root / "configs"), version_base=None):
        source = compose(
    ...
    source.batch_size = 1
    source.gradient_accumulation_steps = 4
    derived = module.training_config(source, tmp_path / "v937", 4, objective=37)
    command = module.training_command(
        tmp_path / "v937", 4, derived, objective=37
    )
    derived_eraf = derived.model.policy_guard.entity_relation_grounding
    assert derived_eraf.grounding_objective_version == 37
    assert derived_eraf.paired_semantic_non_violation_weight == pytest.approx(0.5)
    assert derived_eraf.safe_gain_injector_training_steps == derived.max_steps == 25
    assert derived.save_every == 5
...
def test_v933_roundtrip_records_positive_only_gradient_contract(
    warm_checkpoint, tmp_path
):
    model = model_for(33, ablation="mask_lift_ranking")
    model.prepare_trainable_parameters()
    model.load_checkpoint(warm_checkpoint)
    path = tmp_path / "v933.pt"
    model.save_checkpoint(path, step=50)
    payload = torch.load(path, weights_only=False)
    metadata = payload["architecture_metadata"]
    assert metadata["eraf_grounding_objective_version"] == 33
    assert (
        metadata["eraf_corrective_ranking_gradient_contract"]
        == ablation.POSITIVE_ONLY_RANKING_CONTRACT
    )
    resumed = model_for(33, ablation="mask_lift_ranking")
    resumed.prepare_trainable_parameters()
    resumed.load_checkpoint(path)
    resumed._v930_audit_frozen()


def test_v934_roundtrip_records_universal_positive_only_gradient_contract(
    warm_checkpoint, tmp_path
):
    model = model_for(34, ablation="mask_lift_ranking")
    model.prepare_trainable_parameters()
    model.load_checkpoint(warm_checkpoint)
    path = tmp_path / "v934.pt"
    model.save_checkpoint(path, step=50)
    payload = torch.load(path, weights_only=False)
    metadata = payload["architecture_metadata"]
    assert metadata["eraf_grounding_objective_version"] == 34
    assert metadata["eraf_corrective_ranking_gradient_contract"] is None
    assert (
        metadata["eraf_paired_ranking_gradient_contract"]
        == ablation.UNIVERSAL_POSITIVE_ONLY_RANKING_CONTRACT
    )
    resumed = model_for(34, ablation="mask_lift_ranking")
    resumed.prepare_trainable_parameters()
    resumed.load_checkpoint(path)
    resumed._v930_audit_frozen()


def test_v935_roundtrip_records_bidirectional_semantic_contrast_contract(
    warm_checkpoint, tmp_path
):
    model = model_for(35, ablation="mask_lift_ranking")
    model.prepare_trainable_parameters()
    model.load_checkpoint(warm_checkpoint)
    path = tmp_path / "v935.pt"
    model.save_checkpoint(path, step=50)
    payload = torch.load(path, weights_only=False)
    metadata = payload["architecture_metadata"]
    assert metadata["eraf_grounding_objective_version"] == 35
    assert (
        metadata["eraf_paired_ranking_gradient_contract"]
        == ablation.UNIVERSAL_POSITIVE_ONLY_RANKING_CONTRACT
    )
    assert (
        ...
        == ablation.PAIRED_SEMANTIC_CONTRAST_CONTRACT
    )
    ...
    resumed = model_for(35, ablation="mask_lift_ranking")
    resumed.prepare_trainable_parameters()
    resumed.load_checkpoint(path)
    resumed._v930_audit_frozen()
...
def test_v937_roundtrip_records_soft_action_violation_semantic_contract(
    warm_checkpoint, tmp_path
):
    model = model_for(37, ablation="mask_lift_ranking")
    model.prepare_trainable_parameters()
    model.load_checkpoint(warm_checkpoint)
    path = tmp_path / "v937.pt"
    model.save_checkpoint(path, step=25)
    payload = torch.load(path, weights_only=False)
    metadata = payload["architecture_metadata"]
    assert metadata["eraf_grounding_objective_version"] == 37
    assert (
        metadata["eraf_paired_ranking_gradient_contract"]
        == ablation.UNIVERSAL_POSITIVE_ONLY_RANKING_CONTRACT
    )
    assert (
        metadata["eraf_paired_semantic_contrast_contract"]
        == ablation.SOFT_ACTION_VIOLATION_SEMANTIC_CONTRAST_CONTRACT
    )
    assert metadata["eraf_paired_semantic_contrast_weight"] == pytest.approx(0.1)
    assert metadata["eraf_paired_semantic_contrast_margin"] == pytest.approx(0.1)
    assert metadata["eraf_paired_semantic_non_violation_weight"] == pytest.approx(0.5)
    resumed = model_for(37, ablation="mask_lift_ranking")
    resumed.prepare_trainable_parameters()
    resumed.load_checkpoint(path)
    resumed._v930_audit_frozen()
...
def test_real_action_forward_backward_and_eval_preflight(
    ...
    if objective in {31, 32, 33, 34, 35, 36, 37}:
    ...
    if objective == 37:
        assert metrics["pgc_v937_semantic_non_violation_weight"] == pytest.approx(0.5)
        assert 0.5 <= metrics["pgc_v937_semantic_effective_weight_mean"] <= 1
        assert torch.isfinite(
            torch.as_tensor(
                metrics["loss_pgc_v937_soft_action_violation_semantic_contrast"]
            )
        )
