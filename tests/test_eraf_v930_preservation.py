...
def model_for(objective=30, source=None, ablation="none"):
    return tiny_pgc_fastwam(
        ...
        v9_safe_gain_injector_training_steps=250 if objective in {30, 31, 32, 33, 34} else 0,
        v9_safe_gain_gate_calibration_steps=0,
        v9_safe_gain_noise_levels=2 if objective in {30, 31, 32, 33, 34} else 1,
        v9_cf_ablation=ablation,
        v9_full_goal_action_preservation_weight=1.0 if objective in {31, 32, 33, 34} else 0.0,
        v9_full_goal_token_preservation_weight=0.1 if objective in {31, 32, 33, 34} else 0.0,
        v9_full_goal_context_preservation_weight=1.0 if objective in {31, 32, 33, 34} else 0.0,
    )


@pytest.mark.parametrize("objective", [29, 30, 31, 32, 33, 34])
