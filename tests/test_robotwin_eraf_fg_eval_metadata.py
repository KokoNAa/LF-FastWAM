...
@pytest.mark.parametrize("domain", ["demo_clean", "demo_randomized"])
def test_summarize_without_file_scans_still_rejects_changed_checkpoint(tmp_path, monkeypatch, domain):
    ...
    canonical = catalog/'place_a2b_left'/domain/'correct/episodes.jsonl'
    ...
    for condition in ('correct', 'counterfactual'):
        path = output/'place_a2b_left'/domain/condition; path.mkdir(parents=True)
        (path/'episodes.jsonl').write_text(json.dumps(episode)+'\n')
        (path/'summary.json').write_text(json.dumps({'total_episodes': 1, 'task_config': domain}))
        (path/'initial_states.json').write_text(json.dumps([{'scene_seed': 123, 'sha256': 'same'}]))
        (path/'complete.json').write_text(json.dumps({'complete': True, 'task_config': domain, 'checkpoint': str(checkpoint),
    ...
    monkeypatch.setattr(sys, 'argv', ['eval', 'summarize', '--output', str(output),
        ...
        '--episodes', '1', '--task-config', domain, '--skip-file-hashes'])
    evaluation.main()
    assert json.loads((output/'summary.json').read_text())['episodes'] == 2
    cell_path = output/'place_a2b_left'/domain/'correct/complete.json'
    original = json.loads(cell_path.read_text())
    cell_path.write_text(json.dumps(original | {'task_config': 'demo_randomized' if domain == 'demo_clean' else 'demo_clean'}))
    with pytest.raises(ValueError, match='Mixed evaluation domains'):
        evaluation.main()
    cell_path.write_text(json.dumps(original))
